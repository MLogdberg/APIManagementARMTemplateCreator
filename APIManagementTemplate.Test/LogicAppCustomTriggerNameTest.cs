...
namespace APIManagementTemplate.Test
{
    ...
    public class LogicAppCustomTriggerNameTest
    {
        ...
        public void TestResourcesBackend()
        {
            ...
            Assert.AreEqual("[substring(listCallbackUrl(resourceId(parameters('LogicApp_customtrigger_resourceGroup'), 'Microsoft.Logic/workflows/triggers', parameters('LogicApp_customtrigger_logicAppName'), 'manual'), '2017-07-01').basePath,0,add(10,indexOf(listCallbackUrl(resourceId(parameters('LogicApp_customtrigger_resourceGroup'), 'Microsoft.Logic/workflows/triggers', parameters('LogicApp_customtrigger_logicAppName'), 'manual'), '2017-07-01').basePath,'/triggers/')))]", prop.Value<string>("url"));
            Assert.AreEqual("http", prop.Value<string>("protocol"));
            Assert.AreEqual("[concat('https://management.azure.com/','subscriptions/',subscription().subscriptionId,'/resourceGroups/',parameters('LogicApp_customtrigger_resourceGroup'),'/providers/Microsoft.Logic/workflows/',parameters('LogicApp_customtrigger_logicAppName'))]", prop.Value<string>("resourceId"));
        }
        ...
        public void TestResourcesProperties()
        {
            ...
            var prop = obj["properties"];
            Assert.AreEqual("[listCallbackUrl(resourceId(parameters('LogicApp_customtrigger_resourceGroup'), 'Microsoft.Logic/workflows/triggers', parameters('LogicApp_customtrigger_logicAppName'), 'manual'), '2017-07-01').queries.sig]", prop.Value<string>("value"));
